<template>
  <div align="center">   
    ...
    <v-flex xs5>
      <v-card flat>
        <v-card-title>
          <span class="headline">SOLICITUD CUIS - SUCURSAL </span>
        </v-card-title>
        <v-card-text>
           <v-col cols="8">
            <v-combobox
            ...
            <v-col cols="8">
              <v-text-field v-model="login" label="Usuario de Oficina Virtual" required></v-text-field>
           </v-col>
           <v-col cols="8">
              <v-text-field v-model="password" label="Password de Oficina Virtual" required></v-text-field>
           </v-col>
...
export default {
  ...
  methods: {
    ...
    solicitarCuis() {      
      ...
      solicitudCliente.password = this.password;      
      solicitudCliente.descripcion = login;  //usuario operador
      this.progress = true;

      ApiDosificacionService.solicitudCuis(solicitudCliente)
        .then(response => {
          this.progress = false;
