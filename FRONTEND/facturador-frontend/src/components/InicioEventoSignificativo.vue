    ...
    <v-flex xs6>
      <v-card flat>
        <v-card-title class="text-center justify-center py-6">
          ...
          >Inicio Evento Significativo</h1>
        </v-card-title>
        <v-card-text>
          <v-form ref="form" v-model="valid" lazy-validation>
              <v-col cols="8">
                <v-combobox
                  v-model="parEventoSignificativo"
                  :items="listaParEventoSignificativo"
          ...
          <v-card-actions class="text-xs-center">
            <v-spacer></v-spacer>
            <v-btn :disabled="!valid" color="success" class="mr-4" @click="evento">Inicio Evento Significativo</v-btn>
          </v-card-actions>
...
export default {
  ...
  methods: {
    evento() {
      ...
      // console.log("hora", this.horaEvento);
      
      
      // console.log("xxx", this.fechaEvento.substr(0,4));
      // console.log("xxx", this.fechaEvento.substr(5,2));
      // console.log("xxx", this.fechaEvento.substr(8,2));

      // console.log("xxx", this.horaEvento.substr(0,2));
      // console.log("xxx", this.horaEvento.substr(3,2));
      ...
      // //let x = Date.UTC(anio, mes, dia, hora, minuto);
      // let x = new  Date(anio, mes, dia, hora, minuto).toISOString();
      // console.log("utc", x);
        if (this.$refs.form.validate()) {
            ...
            solicitudCliente.codigoEvento = this.parEventoSignificativo.codigo;
            solicitudCliente.parEventoSignificativo = this.parEventoSignificativo;
            solicitudCliente.login = login;
            solicitudCliente.fechaEnvio = fechaEvento;

            this.progress = true;       
            ApiEventoSignificativoService.inicioEventoSignificativo(solicitudCliente)
            .then(response => {
              this.progress = false;
